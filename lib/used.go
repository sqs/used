...
func (l *LintChecker) LintProblems(prog *loader.Program) (problems []Problem) {
	...
	for _, u := range l.used {
		...
		problems = append(problems, Problem{u.Obj, fmt.Sprintf("%s %s is used %d time%s", typString(u.Obj), name, u.N, plural)})
	}
	return problems
}

func (l *LintChecker) Lint(j *lint.Job) {
	ps := l.LintProblems(j.Program.Prog)
	for _, p := range ps {
		j.Errorf(p.Pos, "%s", p.Text)
	}
}
...
func (c *Checker) Check(lprog *loader.Program) []Used {
	var used []Used
	c.lprog = lprog
	if c.WholeProgram {
	...
	for _, node := range c.graph.nodes {
		...
		if !false {
			for _, pkg2 := range c.lprog.InitialPackages() {
				if pkg2.Pkg == obj.Pkg() {
					pkg = pkg2
					break
				}
		...
		used = append(used, Used{Obj: obj, Position: pos, N: node.usages})
	}
	return used
}
